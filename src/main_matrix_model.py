...
nodeNeighbors = [['Station','Jay-Bergman Field'],['Station','Softball Field'],['Jay-Bergman Field','CFE Arena'],['Softball Field','CFE Arena'],['CFE Arena','Lake Claire'],['CFE Arena', 'Child Center'],['Child Center','Milican Hall'],['Lake Claire','Milican Hall']]
...
def main():
    ...
    try:
        ...
        nop.calc_edge_weights(outGraph,nodeAttr)
    except KeyError:
    ...
    try:
        ...
        for k in outGraph.nodes():
            ...
            if p==True: plt.text(nodePos[k][0],nodePos[k][1]-(edge_avg_weight/6),s=k,fontsize=14,bbox=dict(facecolor='green', alpha=0.5),horizontalalignment='center')
            else: plt.text(nodePos[k][0],nodePos[k][1]-(edge_avg_weight/6),s=k,fontsize=14,bbox=dict(facecolor='gray',alpha=0.5),horizontalalignment='center')
        edge_labels=dict([((u,v,),str(math.floor(d['weight']*100))) for u,v,d in outGraph.edges(data=True)])
    except:
...
if loadFromFile == True:
    ...
    if type(fileType)!=bool: pathDesired = nop.file_excel_interpret(fileDir,fileType,nodeAttr,nodeNeighbors,pathDesired)
